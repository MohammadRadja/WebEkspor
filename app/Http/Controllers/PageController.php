...
class PageController extends Controller
{
    ...
    {
        // Ambil semua jenis tanaman unik sebagai kategori
        $categories = Tanaman::select('jenis')->distinct()->get();

        // Query produk dengan relasi tanaman
        $query = Produk::with('tanaman');

        // Jika user pilih kategori (jenis tanaman)
        if ($request->has('category') && $request->category != '') {
            $query->whereHas('tanaman', function ($q) use ($request) {
                $q->where('jenis', $request->category);
            });
        }
