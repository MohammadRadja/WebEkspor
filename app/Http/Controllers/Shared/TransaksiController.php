...
class TransaksiController extends Controller
{
    ...
    public function cancel(Request $request)
    {
        ...
        try {
            ...
            if (!in_array($transaksi->status, ['menunggu', 'proses'])) {
            ...
            if ($notify) {
                $notify->update([
                    ...
                    'message' => 'Pesanan Anda dengan transaksi #' . $transaksi->id . ' telah dibatalkan.',
                ]);
            }
    ...
    public function cancelByAdmin(Request $request, Transaksi $transaksi)
    {
        $validated = $request->validate([
        ...
        try {
            ...
            // Simpan keterangan admin di notifikasi / log
            $notify = Notifications::where('id_transaksi', $transaksi->id)->latest()->first();
            if ($notify) {
                $notify->update([
                    'type' => 'warning',
                    'title' => 'Pesanan Dibatalkan',
                    'message' => 'Admin membatalkan transaksi ini. Keterangan: ' . ($validated['keterangan'] ?? '-'),
                ]);
            }
