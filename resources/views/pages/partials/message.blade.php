                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-between align-items-center mt-3">
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        {{-- Tombol Bayar --}}
                                        @if ($status === 'proses')
                                            <button type="button" class="btn btn-sm btn-outline-success" data-crud="bayar"
                                                data-url="{{ route('checkout.xendit') }}"
                                                data-id="{{ $notify->transaksi->id }}">
                                                <i class="bi bi-credit-card"></i> Bayar
                                            </button>
                                        @endif

                                        {{-- Tombol Cancel --}}
                                        @if (in_array($status, ['menunggu', 'proses']))
                                            <button type="button" class="btn btn-sm btn-outline-danger" data-crud="cancel"
                                                data-url="{{ route('transaksi.cancel') }}"
                                                data-id="{{ $notify->transaksi->id }}">
