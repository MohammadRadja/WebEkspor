...
@section('content')
    ...
    <section class="py-5 bg-light">
        <div class="container">
            ...
            <!-- Filter Kategori dengan Nav Pills -->
            <ul class="nav nav-pills justify-content-center mb-4">
                <li class="nav-item">
                    <a class="nav-link {{ request('category') == '' ? 'active' : '' }}" href="{{ route('product') }}">
                        Semua
                    </a>
                </li>
                @foreach ($categories as $cat)
                    <li class="nav-item">
                        <a class="nav-link {{ request('category') == $cat->jenis ? 'active' : '' }}"
                            href="{{ route('product', ['category' => $cat->jenis]) }}">
                            {{ ucfirst($cat->jenis) }}
                        </a>
                    </li>
                @endforeach
                ...
                @forelse ($product as $item)
                    <div class="col-md-6 col-lg-3 mb-4">
                        <div class="card h-100 shadow-sm">
                            <img src="{{ asset_or_default($item->gambar) }}" class="card-img-top" alt="{{ $item->nama }}">
